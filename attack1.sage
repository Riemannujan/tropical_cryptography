...
#heuristic_attack
def heuristic_attack(A,B,U,V):
    D = 10 #degré max des polynômes
    i = 1
    while i <= D:
        j = 1
        while j <= D:
...
def multiple_attacks(n,min,max,deg,bool,n_it):
    count = 0
    time = 0
    for i in range(n_it):
        [A,B,U,V,K] = T_stickel(n,min,max,deg,bool)
        t = cputime()
        if heuristic_attack(A,B,U,V)[1] != False:
            count += 1
        time += cputime() - t
    ...
    for i in range(size):
        for j in range(size):
            if A[i,j] == best:
                ...
                best = A[i,j]
                list_of_best = [[i,j]]
    return [best, list_of_best]

def make_simplex_matrix(F, D, h):
    A = zero_matrix((D + 1)**2 + 1, (D + 1)**2 + 1 + 2 * (D + 1))
    for i in range(D):
        for j in range(D):
            A[i * (D + 1) + j + 1, i + 1] = 1
