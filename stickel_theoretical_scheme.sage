...
def stickel_theory_public_key(G):
...
def stickel_theory_private_key(G, public):
	""" In: G: non-abelian finite group
...
def stickel_theory_secret_key(private, V):
...
def TEST_stickel_theoretical(G):
	...
	print("As Alice, time spent to compute each step of Stickel theoretical scheme")
	total_time = 0
	
	# Alice public computation
	t = cputime()
	public = stickel_theory_public_key(G)
	total_time += cputime() - t
	print("public: "+str(cputime() - t))

	Bob = stickel_theory_private_key(G, public)

	# Alice private computation
	t = cputime()
	Alice = stickel_theory_private_key(G, public)
	total_time += cputime() - t
	print("private: "+str(cputime() - t))

	# Alice secret key computation
	t = cputime()
	Kalice = stickel_theory_secret_key(Alice[0], Bob[1])
	total_time += cputime() - t
	print("secret: "+str(cputime() - t))

	# Verification
	Kbob = stickel_theory_secret_key(Bob[0], Alice[1])
	if not Kalice == Kbob:
		return False
